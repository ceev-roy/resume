...
const ProfessionalSummaryForm = ({ data, onChange, setResumeData }) => {
  ...
  const generateSummary = async () => {
    try {
      ...
      setResumeData(prev => ({ ...prev, professional_summary: responce.data.enhancedContent }))
    } catch (error) {
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='mt-6'>
        ...
        <p className='text-xs text-gray-500 max-w-4/5 mx-auto text-center'>Tip: Keep it concise (3-4 sentence) and focus on your most relevant achievements and skills.</p>
      </div>
    </div>
  )
}
