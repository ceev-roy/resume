import { BookTextIcon, FilePenLineIcon, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, UploadCloudIcon, XIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  const [showAnalyseResume, setShowAnalyseResume] = useState(false);
  ...
  const analyseResume = async (event) => {
    event.preventDefault();
    setIsLoading(true)
    try {
      event.preventDefault()
      // const { data } = await api.post('/api/resumes/analyse', { title }, {
      //   headers: {
      //     Authorization: token
      //   }
      // })
      setTitle('')
      setResume(null)
      setShowAnalyseResume(false)
      navigate(`/app/analyse/res123`)
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message)
    }
    setIsLoading(false)
  ...
  return isLoading ? <ScanningLoader /> : (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='flex gap-4'>
          ...
          <button onClick={() => setShowAnalyseResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-green-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
            <BookTextIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-green-300 to-green-500 text-white rounded-full' />
            <p className='text-sm group-hover:text-green-600 transition-all duration-300'>Analyse Resume</p>
          </button>
        </div>
        ...
        {
          showAnalyseResume && (
            <form onSubmit={analyseResume} onClick={() => setShowAnalyseResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
              <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                <h2 className='text-xl font-bold mb-4'>Analyse Resume</h2>
                {/* <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required /> */}
                <label htmlFor="resume-input" className='block text-sm text-slate-700'>
                  Select resume file
                  <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
                ...
                <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors'>Analyse Resume</button>
                <XIcon className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors' onClick={() => { setShowAnalyseResume(false); setTitle("") }} />
              </div>
            </form>
          )
        }
